...
def yolox_inference(img, model, test_size): 
    ...
    tensor_img = torch.from_numpy(tensor_img).unsqueeze(0).float().cuda()
...
def oldtrack_detections_in_roi(bboxes, rois,img):
    roi_flags = [False] * len(rois)  # Reset flags for each frame
    detection_status = {}
    ...
    for i, bbox in enumerate(bboxes):
        for roi_index, roi in enumerate(rois):
            if is_within_roi(bbox, roi):
                ...
                cv2.putText(img, f'Id{roi_index}', (bbox[0] + 10, bbox[1] - 15), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 0), thickness=2)
                break
        else:
            detection_status[i] = (None, False)  # Not inside any ROI
    return detection_status, roi_flags,img

def track_detections_in_roi(bboxes, rois, img):
    roi_flags = [False] * len(rois)  # Reset flags for each frame
    detection_status = {}
    ...
    for i, bbox in enumerate(bboxes):
        for roi_index, roi in enumerate(rois):
            if is_within_roi(bbox, roi):
                ...
                # Ensure bbox coordinates are integers before passing them to putText
                text_position = (int(bbox[0]) + 10, int(bbox[1]) - 15)
                cv2.putText(img, f'Id{roi_index}', text_position, cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 0), thickness=2)
                break
        else:
            detection_status[i] = (None, False)  # Not inside any ROI
    return detection_status, roi_flags, img

def result(img, output_queue, rois):
    # Get predictions
    img = cv2.resize(img, (640, 640))
    bboxes, bbclasses, scores = yolox_inference(img, model, test_size)

    # Track detections in ROIs
    detection_status, roi_flags,img = track_detections_in_roi(bboxes, rois,img)
    ...
    output_queue.put((out_image_rgb, roi_flags))  # Pass updated roi_flags
...
rois = [(15, 150, 85, 600),(100, 120, 585, 210)]
...
while cap.isOpened():
    ...
    if processing_complete:
        ...
        thread = Thread(target=result, args=(img, output_queue, rois))
        thread.start()

    if not output_queue.empty():
        framed, roi_flags = output_queue.get()  # Get both output image and roi_flags
...
print("Final roi_flags : ", roi_flags)
cap.release()
